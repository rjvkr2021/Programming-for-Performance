...
int main(int argc, char* argv[]) {
  ...
  UserKeyValue* h_kvs_insert = new UserKeyValue[ADD];
  // memset(h_kvs_insert, 0, sizeof(UserKeyValue) * ADD); -> 0 se memset kyun?
  uint32_t* h_keys_del = new uint32_t[REM];
  // memset(h_keys_del, 0, sizeof(uint32_t) * REM);
  uint32_t* h_keys_lookup = new uint32_t[FIND];
  ...
  // Read data from file
  uint32_t* tmp_keys_insert = new uint32_t[ADD];
  read_data(path_insert_keys, ADD, tmp_keys_insert);
  uint32_t* tmp_values_insert = new uint32_t[ADD];
  ...
  if (REM > 0) {
    uint32_t* tmp_keys_delete = new uint32_t[REM];
  ...
  if (FIND > 0) {
    uint32_t* tmp_keys_search = new uint32_t[FIND];
  ...
  float total_search_time = 0.0F;
	...
	// runs wale loop ke andar initialize krna hai kya? -> I don't think so.
	HashTable ht;
	bool *insert_result = (bool*)malloc(ADD * sizeof(bool));
	bool *delete_result = (bool*)malloc(REM * sizeof(bool));
	uint32_t *lookup_result = (uint32_t*)malloc(FIND * sizeof(uint32_t));

  HRTimer start, end;
  uint32_t del_runs = 0, search_runs = 0;
  for (uint32_t i = 0; i < runs; i++) {
    start = HR::now();
    ht.batch_insert(ADD, h_kvs_insert, insert_result);
    end = HR::now();
    ...
    if (REM > 0) {
      start = HR::now();
      ht.batch_delete(REM, h_keys_del, delete_result);
      end = HR::now();
    ...
    if (FIND > 0) {
      start = HR::now();
      ht.batch_search(FIND, h_keys_lookup, lookup_result);
      end = HR::now();
